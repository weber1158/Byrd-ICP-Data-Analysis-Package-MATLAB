...
elseif dataset == "kilimanjaro"
	...
	elseif subset == "d18O-decade"
		T=readtable(url,"Delimiter"," ",...
					...
					"ConsecutiveDelimitersRule","join",...
					"ReadVariableNames",false);
		% Delete footer rows and end columns
		T(151:end,:)=[];
		T(:,[1,8:end])=[];
		% Create copy of table and split by the gap in the data
		T2 = T;
		T2_top = T2(1:92,:);
		T2_end = T2(101:end,:);
		T2_mid = [...
			0.92 -10.09 NaN -9.70 -9.44 NaN;...
			0.93  -8.23 NaN -9.75 -9.45 NaN;...
			0.94  -8.65 NaN -9.62 -9.31 NaN;...
			0.95  -8.98 NaN -9.42 -9.28 NaN;...
			0.96  -7.58 NaN -9.24 -9.15 NaN;...
			0.97 -10.74 NaN -9.87 -9.25 NaN;...
			0.98 -11.77 NaN -9.41 -9.24 NaN;...
			0.99  -9.03 NaN -9.28 -9.13 NaN]; % Fixing the gap in the middle of T2_top and T2_end
		% Reform into table
		T2_mid = array2table(T2_mid);
		T2_mid.Properties.VariableNames={'Var2','Var3','Var4','Var5','Var6','Var7'};
		T = [T2_top; T2_mid; T2_end];
		% Add variable names
